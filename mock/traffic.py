    ...
    if v1 > v2:
        if a1 == a2:
            ...
            if v_rel ** 2 + 2 * a_rel * (dist - minGap) >= 0:
                ttc = (-v_rel + (v_rel ** 2 + 2 * a_rel *
                       (dist - minGap)) ** 0.5) / a_rel
            else:
                ttc = 1e6
    else:
...
def getNCCTlsInfo1(vehID):  # 用于替代 get_tls_info()
    ...
    if (len(tlsInfo)) != 0:
        ...
        currPhaseDuration = traci.trafficlight.getNextSwitch(
            tlsID) - traci.simulation.getTime()
        tlsPhases = traci.trafficlight.getCompleteRedYellowGreenDefinition(tlsID)[
            0].phases
...
def get_tls_info(veh_id):
    ...
    if len(tls_info) != 0:
        ...
        current_phase_duration = traci.trafficlight.getNextSwitch(
            tls_id) - traci.simulation.getTime()
        # tls_phases存储四个元组 Phase(duration=4.0, state='ryry', minDur=4.0, maxDur=4.0, next=(), name='wey_nsr')
        tls_phases = traci.trafficlight.getCompleteRedYellowGreenDefinition(tls_id)[
            0].phases
        ...
        if tls_phase_name == "wer_nsg":  # 东西向红灯，南北向绿灯
            ...
            time_to_green_we = current_phase_duration + \
                tls_phase_dict['wer_nsy']
        elif tls_phase_name == "wer_nsy":  # 东西向红灯，南北向黄灯
            time_to_green_ns = current_phase_duration + \
                tls_phase_dict['weg_nsr'] + tls_phase_dict['wey_nsr']
            time_to_green_we = current_phase_duration
        elif tls_phase_name == "weg_nsr":  # 东西向绿灯，南北向红灯
            time_to_green_ns = current_phase_duration + \
                tls_phase_dict['wey_nsr']
            time_to_green_we = current_phase_duration
        else:  # 东西向黄灯，南北向红灯
            time_to_green_ns = current_phase_duration
            time_to_green_we = current_phase_duration + \
                tls_phase_dict['wer_nsg'] + tls_phase_dict['wer_nsy']
...
def get_IDM_speed(veh_id, glosa_flag=True, speed_flag=True):
    ...
    if tl_info:
        if glosa_flag:
            target_speed, min_speed, max_speed = get_glosa_speed(
                veh_id, alpha=0.9)      # IDM中的target speed
        if l[0] == 'l':
            # 前方没有车辆的情况
            next_a = max_a * (1 - (v1 / (target_speed + 1e-6)) ** 4)
        else:
            # 前方有其他车辆的情况
            actual_gap = traci.vehicle.getDistance(
                l[0]) - traci.vehicle.getDistance(veh_id) - 4.9
            v2 = traci.vehicle.getSpeed(l[0])
            desired_gap = s0 + max(0, v1 * T + v1 *
                                   (v1 - v2) / (2 * (max_a * b) ** 0.5))
            next_a = max_a * (1 - (v1 / (target_speed + 1e-6))
                              ** 4 - (desired_gap / actual_gap) ** 2)
    else:
        if l[0] == 'l':
            # 前方没有车辆的情况
            next_a = max_a * (1 - (v1 / (target_speed + 1e-6)) ** 4)
            ...
            if tls_phase_name == 'weg_nsr':
                ...
                if dist_to_intersec / 20. <= 0.9 * time_to_green_we:
                    ...
                    return [1,
                            time_to_green_we +
                            tls_phase_dict['wey_nsr'] +
                            tls_phase_dict['wer_nsg'] +
                            tls_phase_dict['wer_nsy'],
                            time_to_green_we +
                            tls_phase_dict['wey_nsr'] + tls_phase_dict['wer_nsg'] +
                            tls_phase_dict['wer_nsy'] +
                            tls_phase_dict['weg_nsr'],
                            dist_to_intersec,
                            tls_phase_dict]
            else:
...
def get_safe_speed(veh):
    ...
    if l[0] != 'l':
        dist_to_leader = traci.vehicle.getDistance(
            l[0]) - traci.vehicle.getDistance(veh) - 4.9
        leader_speed = traci.vehicle.getSpeed(l[0])
        safe_speed = leader_speed + \
            (dist_to_leader - leader_speed * 1) / \
            ((curr_speed + leader_speed) / (2 * 3) + 1.)
        print(safe_speed)
        return safe_speed


def getCurrentStateNCC():
    ...
    for veh in veh_list:
        ...
        if tl_info2:
            ...
            #############################################################
            current_state_dict[veh].append(
                traci.vehicle.getSpeed(veh))             # 当前车的速度
            current_state_dict[veh].append(
                l_speed)                                 # 前车的速度
            current_state_dict[veh].append(
                f_speed)                                 # 后车的速度
            ############################################################
            current_state_dict[veh].append(
                dist_to_cl)                             # 当前车距离前车的距离
            current_state_dict[veh].append(
                dist_to_cf)                             # 当前车和后车的距离
            #############################################################
            ...
            current_state_dict[veh].append(
                tl_dist)                                # 距离交叉路口的距离
            # 通过交叉路口的最小时间
            current_state_dict[veh].append(tl_min)
            # 通过交叉路口的最大时间
            current_state_dict[veh].append(tl_max)
            #############################################################
